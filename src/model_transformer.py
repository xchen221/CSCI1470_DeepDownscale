...
class TransformerBottleneck(nn.Module):
    def __init__(self, embed_dim=64, num_heads=4, num_layers=1, mlp_ratio=4.0, dropout=0.1):
        super().__init__()
...
class UNetTransformerDownscale(nn.Module):
    def __init__(
        ...
        self.down2 = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(base_channels * 2, base_channels * 4)              # 64 -> 128
        )
        self.down3 = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(base_channels * 4, embed_dim)                      # 128 -> 64 (or embed_dim)
        )

        # Transformer bottleneck at deeper, smaller spatial resolution
        ...
        # Decoder
        self.up1 = nn.ConvTranspose2d(embed_dim, base_channels * 4, kernel_size=2, stride=2)
        self.conv_up1 = DoubleConv(base_channels * 8, base_channels * 4)  # 128 + 128
    ...
    def forward(self, x):
        ...
        x4 = self.down3(x3)    # [B, embed_dim, H/8, W/8]
