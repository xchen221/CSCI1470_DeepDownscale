...
def main():
    ...
    save_path = "outputs/checkpoints/best_unet_t_elev_urban.pt"
    history_path = "outputs/unet_t_elev_urban_loss_history.csv"
    # save_path = "outputs/checkpoints/best_unet_gn_t_elev_urban.pt"
    # history_path = "outputs/unet_gn_t_elev_urban_loss_history.csv"

    device = get_device()
    print("Using device:", device)

    train_ds = DownscaleDatasetTElevUrban(train_path, topo_path, urban_path)
    val_ds = DownscaleDatasetTElevUrban(val_path, topo_path, urban_path)

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0)

    model = UNetDownscale(in_channels=3, out_channels=1).to(device)
    #model = UNetDownscaleGN(in_channels=3, out_channels=1).to(device)
